...
- `/submit-time` is a two-step flow: one captain proposes a time, the other confirms (or clears it). Once confirmed, the bot posts a claim card to the assignments channel so casters/refs/mods can claim the match and gain channel access when they accept.
...
6. **Submit match time** with `/submit-time` from inside the match channel (captains/co-caps). One captain proposes the time; the opposing captain runs the command to confirm or clear it. On confirmation, the bot posts a claim card to the assignments channel and pings staff in the alert channel so casters/refs/mods can accept and gain channel access for that specific match.
...
| `/submit-time <scheduled_for>` | Captains & co-captains | Run inside the match channel to propose a match time; the opposing captain runs it again to confirm or change. On confirmation, the bot pings staff in the alert channel and posts a claim card to the assignments channel so casters/refs/mods with the right role can accept and gain access. |
