...
5. **Run weekly matches** via `/admin-create-match`: the bot spins up `[team1]-vs-[team2]` channels in your match category, pings staff in the alert channel, and schedules mid-week + deadline reminders (Monday→Monday windows).
6. **Submit scores** with `/submit-scores` inside the match channel (captains/co-caps). Both sides must match within three attempts; success locks the channel, posts the plain-text result to the results channel, and updates Challonge when configured. If a forfeit or dispute needs an override, admins can use `/admin-submit-scores` to finalize the result instantly.
